...
function buildDeps() {
  ...
  return { prisma, audit, inventory, tx };
}
...
describe("SalesService", () => {
  describe("checkout — FR-SAL-1/2/3 + FR-INV-2", () => {
    it("computes totals, deducts stock through InventoryService, and records a completed order", async () => {
      const { prisma, audit, inventory, tx } = buildDeps();
      prisma.store.findUnique.mockResolvedValue({ id: "store-1", vatRate: 14 });
      ...
      const service = new SalesService(prisma as any, audit as any, inventory as any);
      ...
      prisma.productVariant.findMany.mockResolvedValue([
        {
          ...
          barcode: "12345",
          sellingPriceOverride: null,
          product: { modelName: "Runner 300", baseSellingPrice: 500, baseCostPrice: 300 },
      ...
      prisma.productVariant.findMany.mockResolvedValue([
        {
          ...
          product: { modelName: "Sandal", baseSellingPrice: 400 },
